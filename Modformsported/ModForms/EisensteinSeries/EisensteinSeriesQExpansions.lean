...
import Modformsported.ModForms.EisensteinSeries.QExpAux
...
theorem eisen_iden (k : ℕ) (hk : 3 ≤ (k : ℤ)) (z : ℍ) :
  ...
  rw [eisensteinSeriesIsModularForm]
  simp_rw [eisensteinIsSlashInv]
  rfl
...
theorem sum_sigmaFn_eqn {k : ℕ} (z : ℍ) :
  ...
  simp only [Finset.univ_eq_attach, cpow_nat_cast, uhc, Nat.cast_sum, Nat.cast_pow, Nat.isUnit_iff] at *
  simp_rw [mul_assoc] at *
  norm_cast at *
  simp at *
  have dma := div_mul_aux k z n
  simp only [Nat.isUnit_iff, cpow_nat_cast, uhc] at dma 
  rw [dma] at this
  have hr :
...
theorem a1 {k : ℕ} (e : ℕ+) (z : ℍ) :
  ...
  simp only [uhc, cpow_nat_cast] at *
  apply HH.subtype 
...
theorem Eisenstein_Series_q_expansion (k : ℕ) (hk : 3 ≤ (k : ℤ)) (hk2 : Even k) (z : ℍ) :
  ...
  have t4 :
    ...
    by congr; funext c; rw [t2 (c • z : ℍ)]; rw [auxnpm c z]; rw [← mul_assoc]
  simp only [uhc, Int.cast_ofNat, cpow_nat_cast, one_div, neg_mul, ge_iff_le] at *
  rw [this]
  simp [auxnpm,  one_div, ofReal_mul, Int.cast_neg, 
  ...
  · rw [summable_mul_prod_iff_summable_mul_sigma_antidiagonall]
    ...
    apply this.congr
    intro b
    simp only [ge_iff_le, cpow_nat_cast, uhc]
    norm_cast
  · intro e
    have := @a1 k e z
    simp at *
...
example : ( 1 : ℂ) + 1 = 2 := by exact one_add_one_eq_two
...
theorem Eisen_4_non_zero : G[(4 : ℕ)] ≠ 0 := by
  ...
  simp only [Nat.cast_ofNat,
    ge_iff_le, Nat.succ_sub_succ_eq_sub, tsub_zero, uhc] at H3 
  rw [r1] at H3 
  ...
  let ε :=
    2 * rZ 4 +
      2 * (16 * π ^ 4 / ↑((2 + 1) * 2)) * ∑' n : ℕ+, ↑(sigmaFn 3 n) * Real.exp (-(2 : ℕ)  * π * ↑n)
